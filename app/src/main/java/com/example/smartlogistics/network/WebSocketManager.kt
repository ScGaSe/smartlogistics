...
class WebSocketManager(
        ...
        webSocket = client.newWebSocket(request, object : WebSocketListener() {
            ...
            override fun onMessage(webSocket: WebSocket, text: String) {
                ...
                try {
                    ...
                    if (message.type == "location" || message.type == "share_ended") {
                        scope.launch {
    ...
    fun sendShareEnded() {
        if (_connectionState.value != ConnectionState.Connected) {
            Log.w(TAG, "Cannot send share_ended: not connected")
            return
        }

        val message = mapOf(
            "type" to "share_ended",
            "timestamp" to java.text.SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss", java.util.Locale.getDefault())
                .format(java.util.Date())
        )

        val json = gson.toJson(message)
        Log.d(TAG, "Sending share_ended: $json")
